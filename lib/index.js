...
module.exports = (function () {
  ...
  Workbook.prototype.deleteSheet = function (sheetName) {
    ...
    var rel = self.workbookRels.find(
  ...
  Workbook.prototype.copySheet = function (sheetName, copyName) {
    ...
    var newSheetIndex = (
      self.workbook.findall("sheets/sheet").length + 1
    ).toString();
  ...
  Workbook.prototype._rebuild = function () {
    ...
    self.archive.file(
      self.prefix +
        ...
        "_rels" +
        "/" +
        path.basename(self.workbookPath) +
        ".rels",
      etree.tostring(self.workbookRels)
    );
    self.archive.file(self.workbookPath, etree.tostring(self.workbook));
    self.sheets = self.loadSheets(
      self.prefix,
      self.workbook,
      self.workbookRels
    );
  ...
  Workbook.prototype.loadTemplate = function (data) {
    ...
    self.workbook = etree
      .parse(self.archive.file(workbookPath).asText())
      .getroot();
    self.workbookRels = etree
      .parse(
        self.archive
          .file(
            self.prefix +
    ...
    self.calChainRel = self.workbookRels.find(
    ...
    self.sharedStringsPath =
      self.prefix +
      "/" +
      self.workbookRels.find(
        "Relationship[@Type='" + SHARED_STRINGS_RELATIONSHIP + "']"
      ).attrib.Target;
    self.sharedStrings = [];
    etree
      .parse(self.archive.file(self.sharedStringsPath).asText())
      .getroot()
      .findall("si")
      .forEach(function (si) {
        var t = { text: "" };
  ...
  Workbook.prototype.substitute = function (sheetName, substitutions) {
    ...
    if (dimension) {
      if (totalRowsInserted > 0 || totalColumnsInserted > 0) {
        ...
        dimensionEndRef.col = self.numToChar(
          self.charToNum(dimensionEndRef.col) + totalColumnsInserted
        );
  ...
  Workbook.prototype.loadSheet = function (sheet) {
    ...
    for (var i = 0; i < self.sheets.length; ++i) {
      if (
        (typeof sheet === "number" && self.sheets[i].id === sheet) ||
        self.sheets[i].name === sheet
      ) {
        info = self.sheets[i];
        break;
      }
  ...
  Workbook.prototype.loadDrawing = function (sheet, sheetFilename, rels) {
    ...
    var relationshipId = drawingPart.attrib["r:id"],
      target = rels.find("Relationship[@Id='" + relationshipId + "']").attrib
        .Target,
    ...
    drawing.relRoot = etree
      .parse(self.archive.file(drawing.relFilename).asText())
      .getroot();
    return drawing;
  };
  ...
  Workbook.prototype._moveTwoCellAnchor = function (
    drawingElement,
    fromRow,
    nbRow
  ) {
  ...
  Workbook.prototype.substituteHyperlinks = function (
    sheetFilename,
    substitutions
  ) {
    let self = this;
  ...
  Workbook.prototype.substituteTableColumnHeaders = function (
    tables,
    substitutions
  ) {
    var self = this;
  ...
  Workbook.prototype.charToNum = function (str) {
    var num = 0;
    for (
      var idx = str.length - 1, iteration = 0;
      idx >= 0;
      --idx, ++iteration
    ) {
  ...
  Workbook.prototype.substituteScalar = function (
    cell,
    string,
    placeholder,
    substitution
    ...
    if (substitution.length === 0) {
      ...
      substitution.forEach(function (element, idx) {
        ...
        if (idx === 0) {
          ...
          if (value instanceof Array) {
            newCellsInsertedOnNewRow = self.substituteArray(
              newCells,
              newCell,
              value
            );
  ...
  Workbook.prototype.getCurrentCell = function (
    cell,
    currentRow,
    cellsInserted
  ) {
  ...
  Workbook.prototype.pushRight = function (
    workbook,
    sheet,
    currentCell,
    numCols
  ...
  Workbook.prototype.pushDown = function (
    workbook,
    sheet,
    tables,
    currentRow,
